...
class BusinessWaveController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'code' => 'nullable|string|max:255|unique:business_waves,code',
            ...
            'code.unique' => 'Ce code existe déjà dans la base de données. Une vague Business avec ce code a déjà été enregistrée. Veuillez vérifier votre liste de vagues ou utiliser un code différent pour éviter les doublons.',
            'start_date.required' => 'La date de début est obligatoire. Veuillez sélectionner une date.',
            'start_date.date' => 'La date de début doit être une date valide.',
            'status.string' => 'Le statut doit être une chaîne de caractères.',
            'status.in' => 'Le statut doit être l\'un des suivants : brouillon, ouverte, clôturée.',
        ...
        try {
            ...
            if (empty($code)) {
                ...
                $counter = 1;
                $originalCode = $code;
                while (BusinessWave::where('code', $code)->exists()) {
